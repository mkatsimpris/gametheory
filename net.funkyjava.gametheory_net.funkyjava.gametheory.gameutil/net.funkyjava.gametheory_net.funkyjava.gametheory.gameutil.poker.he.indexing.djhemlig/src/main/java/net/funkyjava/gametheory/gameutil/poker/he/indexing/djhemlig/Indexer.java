...
abstract public class Indexer implements CardsGroupsIndexer {
	...
	private void countOffsets() {
		...
		for (int i = 1; i < numRankPattern.length; i++) {
			cumOffsets[i] = cumOffsets[i - 1] + numRankPattern[i - 1]
					* rankPatternSuits[i - 1].getSize();
		}
	}
	...
	private int tableIndex(int Rank[], int Suit[]) {
		...
		int index = cumOffsets[rankIsoIndex] + rankPositionMap[rankidx]
				* rankPatternSuits[rankIsoIndex].getSize() + suitIndex;
