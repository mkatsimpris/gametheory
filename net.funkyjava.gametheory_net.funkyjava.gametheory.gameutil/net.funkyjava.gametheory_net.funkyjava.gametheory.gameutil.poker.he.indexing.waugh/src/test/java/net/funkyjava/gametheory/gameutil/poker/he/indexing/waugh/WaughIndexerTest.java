...
public class WaughIndexerTest {
					...
					if (c3 != c1 && c3 != c2)
						for (int c4 = c3 + 1; c4 < 52; c4++)
							if (c4 != c2 && c4 != c1) {
								...
								assertTrue(
										"Indexer's index/unindex doesn't return an equivalent set for cards "
												+ c1 + " " + c2 + " " + c3
												+ " " + c4,
										areEquivalent(toIndex, res, perm));
							}
	...
	private static boolean permute(java.util.List<Integer> arr, int k,
			int[][] src, int[][] res) {
		...
		if (k == arr.size() - 1) {
			return src[0][arr.get(0)] == res[0][0]
					&& src[0][arr.get(1)] == res[0][1]
					&& src[0][arr.get(2)] == res[0][2]
					&& src[0][arr.get(3)] == res[0][3]
					&& src[1][arr.get(0)] == res[1][0]
					&& src[1][arr.get(1)] == res[1][1]
					&& src[1][arr.get(2)] == res[1][2]
					&& src[1][arr.get(3)] == res[1][3];
		}
		return false;
	}
