...
public class Cards52Strings {
	 ...
	 * 
	 * @param cardStr
	 *            String representation of the card : "2c", ..., "As"
	...
	/**
	 * Return multiple cards as an int array. Cards must be represented with no
	 * space, according to standard representation, eg : "AdAc2hJd"
	 * 
	 * @param cardsStr
	...
	public int[] getCards(String cardsStr) {
		checkArgument(cardsStr.length() % 2 == 0,
				"Wrong card string size, must be multiple of 2");
	 ...
	 * 
	 * @param rank
	 *            String representation of the rank : "2", "3",..., "9", "T",
	 *            "J", "Q", "K", "A"
	 * @return the standard rank represented by the String
	 */
	public static int getStandardRank(@NonNull String rank) {
		checkArgument(rank.length() == 1,
				"Cannot translate String rank of length != 1");
		for (int i = 0; i < ranks.length; i++)
			if (ranks[i].equals(rank))
				return i;
		throw new IllegalArgumentException("Provided rank '" + rank
				+ "' is not a valid one");
	}
	 ...
	 * 
	 * @param color
	 *            String representation of the color : "c", "d", "h", "s"
	 * @return the standard color represented by the String
	 */
	public static int getStandardColor(@NonNull String color) {
		checkArgument(color.length() == 1,
				"Cannot translate String color of length != 1");
		for (int i = 0; i < colors.length; i++)
			if (colors[i].equals(color))
				return i;
		throw new IllegalArgumentException("Provided color '" + color
				+ "' is not a valid one");
	}
